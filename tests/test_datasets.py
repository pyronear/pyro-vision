...
def test_openfire(tmpdir_factory):
    ...
    train_set = datasets.OpenFire(
        root=root, train=True, download=True, num_samples=num_samples, img_folder=Path(img_folder)
    )
    ...
    assert all(sample["name"].rpartition(".")[-1] in ["jpg", "jpeg", "png", "gif"] for sample in train_set.data)
    assert all(sample["name"].rpartition(".")[-1] in ["jpg", "jpeg", "png", "gif"] for sample in test_set.data)
    ...
    assert abs((len(train_set) + len(test_set)) - len(extract)) <= 33
