...
class SubSamplerDataset(Dataset):
    
    def __init__(self, metadata, path_to_frames,frame_per_seq,target_names=None,train=False, transform=None):
    ...
    def computeSubSet(self,bs):
        meta=self.metadata
   
        self.SubSetImgs=[]
        #self.SubFire=[]
        #print(self.uniqueSEQ)
        #print(self.seq)
        ...
        for seU in self.uniqueSEQ:
            ...
            self.SubSetImgs+=nn
            if(meta[meta['seq']==seU]['fire'].values[0]==0 and random.randint(0,1)==1):
                nn = [self.imgs[i] for i,se in enumerate(self.seq) if se==seU]
                if(len(nn)>self.frame_per_seq):
                    nn=random.sample(nn, self.frame_per_seq)
                nb = [float(frame.split("frame", 1)[1].split(".", 1)[0]) for frame in nn]
                nb,nn = (list(t) for t in zip(*sorted(zip(nb,nn))))
                self.SubSetImgs+=nn
                   # ff = [meta[meta['SEQ']==seU]['fire'].values[0] for frame in nn]
                    #self.SubFire+=ff

        
        #self.SubSetImgs=self.SubSetImgs[:len(self.SubSetImgs)-len(self.SubSetImgs)%(self.frame_per_seq*bs)] 
        #self.SubFire=self.SubFire[:len(self.SubSetImgs)] 
    ...
    def __getitem__(self, index):
        ...
        #print(img,str(meta['fire'].values[0]))
        ...
        return observation, meta['fire'].values[0]
...
class WildFireSplitter:
    ...
    def fit(self, wildfire, batch_size):
    ...
    def set_splits(self, dataframes):
        ...
        for set_ in ['train', 'val', 'test']:
            self.splits[set_] = SubSamplerDataset(metadata=dataframes[set_],
                                                path_to_frames=self.wildfire.path_to_frames,
                                                target_names=self.wildfire.target_names,
                                                frame_per_seq=self.wildfire.frame_per_seq,
                                                transform=self.transforms[set_])
           
            self.splits[set_].computeSubSet(self.batch_size)
